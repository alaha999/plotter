...
def CreateRatioPad(padName,padYlow,padYhigh):
    #padYhigh=doRatio
    pad = ROOT.TPad(padName, padName, 0, padYlow, 1,padYhigh);
    gStyle.SetOptStat(0)
    pad.SetLeftMargin(0.15)
    pad.SetRightMargin(0.20)
...
def SetRatioHistoStyle(ratiohist,xlabel=""):
    ...
    ratiohist.GetXaxis().SetNdivisions(513);
    ratiohist.GetXaxis().SetLabelSize(18)
    ...
    ratiohist.GetYaxis().SetLabelSize(18)
...
def SetStackHistoStyle(stackhist,xlabel=""):
    ##X-axis
    stackhist.GetXaxis().SetTitle(xlabel)
    stackhist.GetXaxis().SetTitleSize(20)
    stackhist.GetXaxis().SetTitleFont(43)
    stackhist.GetXaxis().SetTitleOffset(1.15)
    stackhist.GetXaxis().SetLabelFont(43)
    stackhist.GetXaxis().SetLabelSize(20)
    stackhist.GetXaxis().CenterTitle()
    ...
    stackhist.GetYaxis().SetLabelSize(18)
...
def CMSLabel(pad):
    ...
    if(TakeLegendOutside==True):
        ...
        if(year==2017):
            text.DrawLatex(pad.GetUxmax()-0.47, 0.92, "#sqrt{{s}} = 13 TeV, {:.1f} fb^{{-1}}".format(41.6))    
    else:
        if(year==2016):
